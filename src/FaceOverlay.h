...
private:
    ...
    static void divide(const glm::vec3& a, const glm::vec3& b,
        ...
        if(determinant(glm::vec2(left), glm::vec2(right), glm::vec2(avg)) < 0) {
            top.addVertex(a);
            top.addVertex(b);
        } else {
            bottom.addVertex(a);
...
public:
    ...
    void draw(ofxFaceTracker& tracker) {
        ...
        //std::cout << "eyeLeftLidLeft: " << tracker.getImagePoint(36) << endl;
        glm::vec2 eyeCenterLeft = glm::mix(eyeLeftLidLeft, eyeLeftLidRight, .5);
        //std::cout << "eyecenterleft: " << eyeCenterLeft << endl;
